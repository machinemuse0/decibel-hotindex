...
fn run_serving_bench(params: &BenchParams<'_>) -> Result<BenchmarkReport> {
    ...
    match params.backend {
        ...
        "rocksdb" => Err(HotIndexError::Config(
            "RocksDB benchmark must be run from the main worktree".to_string(),
        )),
        "toplingdb" => run_toplingdb_serving(params, query_corpus, corpus),
...
fn run_toplingdb_serving(
    ...
    let path = params
        .db_path
        .map(PathBuf::from)
        .unwrap_or_else(|| params.dataset.join("materialized/toplingdb"));
    let engine = ToplingDbEngine::open(&path)?;
    let compaction_performed = maybe_compact_toplingdb(&engine, params.compact_before_run)?;
    let result = measure_queries(&engine, &corpus, params.query_measure_config())?;
    build_report(ReportInput {
        manifest: params.manifest,
        dataset: params.dataset,
        ...
        storage_state: toplingdb_storage_state(
            &path,
            params.compact_before_run,
...
fn run_ingest_bench(params: &BenchParams<'_>) -> Result<BenchmarkReport> {
    match params.backend {
        ...
        "toplingdb" => run_toplingdb_ingest(params),
        other => Err(HotIndexError::Config(format!(
...
#[cfg(feature = "toplingsdb")]
fn run_toplingdb_ingest(params: &BenchParams<'_>) -> Result<BenchmarkReport> {
    let rows = IngestRows::load(params.dataset)?;
    let path = params
        .db_path
        .map(PathBuf::from)
        .unwrap_or_else(|| params.dataset.join("materialized/toplingdb-bench-ingest"));
    let engine = ToplingDbEngine::open(&path)?;
    warmup_ingest(&engine, &rows, params.warmup)?;
    let compaction_performed = maybe_compact_toplingdb(&engine, params.compact_before_run)?;
    let result = measure_ingest(&engine, &rows, params.iterations)?;
...
#[cfg(not(feature = "toplingsdb"))]
fn run_toplingdb_ingest(_params: &BenchParams<'_>) -> Result<BenchmarkReport> {
    Err(HotIndexError::Config(
        "ToplingDB benchmark requires `--features toplingsdb`".to_string(),
    ))
}
...
fn run_read_under_ingest_bench(params: &BenchParams<'_>) -> Result<BenchmarkReport> {
    ...
    match params.backend {
        ...
        "toplingdb" => run_toplingdb_read_under_ingest(params, query_corpus, corpus, rows),
        other => Err(HotIndexError::Config(format!(
...
fn run_toplingdb_read_under_ingest(
    ...
    let path = params.db_path.map(PathBuf::from).unwrap_or_else(|| {
        params
            .dataset
            .join("materialized/toplingdb-bench-read-ingest")
    });
    let engine = ToplingDbEngine::open(&path)?;
    let compaction_performed = maybe_compact_toplingdb(&engine, params.compact_before_run)?;
...
fn build_report(input: ReportInput<'_>) -> Result<BenchmarkReport> {
    ...
    Ok(BenchmarkReport {
        ...
        result: input.result,
        disclaimer: "engineering smoke benchmark only; publishable claims require a pinned release build, checksum-passed backend comparison, and documented environment/storage state".to_string(),
    })
}
...
fn toplingdb_storage_state(
    ...
    let mut backend_options = BTreeMap::new();
    backend_options.insert("backend".to_string(), "toplingdb".to_string());
    backend_options.insert("cargo_feature".to_string(), "toplingsdb".to_string());
    backend_options.insert(
        "config_env".to_string(),
        "TOPLINGDB_EASY_MIGRATE_CONF".to_string(),
    );
    backend_options.insert("worktree".to_string(), "topingdb".to_string());
    StorageStateReport {
        path: path.display().to_string(),
        compaction: CompactionReport {
            requested: compaction_requested,
...
#[cfg(feature = "toplingsdb")]
fn maybe_compact_toplingdb(engine: &ToplingDbEngine, compact_before_run: bool) -> Result<bool> {
...
fn print_usage() {
    eprintln!(
        "usage:
  decibel-hotindex-bench run --dataset <dataset-dir> --engine memory --class serving --workload mixed_market_dashboard --iterations <n> --warmup <n> --concurrency <workers> --access-pattern sequential|uniform|zipfian --seed <seed> --rate <qps> --cache-state unspecified|warm|cold-cleared --out <report.json> [--allow-failures]
  decibel-hotindex-bench run --features toplingsdb --dataset <dataset-dir> --engine toplingdb --db-path <toplingdb-path> --class serving --workload mixed_market_dashboard --iterations <n> --warmup <n> --concurrency <workers> --access-pattern sequential|uniform|zipfian --seed <seed> --rate <qps> --compact-before-run --cache-state unspecified|warm|cold-cleared --cache-clear-command <cmd> --out <report.json> [--allow-failures]
...
fn write_markdown_summary(path: &Path, reports: &[BenchmarkReport]) -> Result<()> {
    ...
    text.push_str(
        "- disclaimer: engineering smoke benchmark only; publishable claims require a pinned release build, checksum-passed backend comparison, and documented environment/storage state\n\n",
    );
