    ...
    match engine {
        ...
        "rocksdb" => {
            return Err(HotIndexError::Config(
                "RocksDB replay must be run from the main worktree".to_string(),
            ));
        }
        #[cfg(feature = "toplingsdb")]
        "toplingdb" => {
            ...
            replay_toplingdb_materialized(&dataset, db_path)?;
        }
...
#[cfg(feature = "toplingsdb")]
fn replay_toplingdb_materialized(dataset: &Path, db_path: PathBuf) -> Result<()> {
    if db_path.exists() {
        return Err(HotIndexError::Config(format!(
            "toplingdb replay db path already exists: {}; remove it or choose a new --db-path",
            db_path.display()
        )));
    }
    let staging_path = staging_materialized_path(&db_path, "toplingdb")?;
    if staging_path.exists() {
        return Err(HotIndexError::Config(format!(
            "toplingdb replay staging path already exists: {}; remove stale staging output before retrying",
    ...
    let result = (|| {
        let engine = ToplingDbEngine::open(&staging_path)?;
...
mod tests {
    use super::{
        ...
        DecibelEventType, FillRow, HotIndexError, Network, NormalizedEvent, QueryCorpusRows,
        QueryKind, SyntheticDataset, NORMALIZED_ARTIFACTS, QUERY_CORPUS_ARTIFACTS,
    };
