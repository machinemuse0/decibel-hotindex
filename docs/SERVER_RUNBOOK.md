...
`scripts/toplingdb-cargo.sh`. The wrapper supplies Darwin compatibility for the
pinned `rust-toplingdb` revision and applies a narrow, idempotent patch to the
Cargo git checkout. Linux benchmark servers should still use the same wrapper so
the command shape remains identical across environments.
...
Run `rtk ./scripts/toplingdb-preflight.sh` from the `topingdb` worktree before
building benchmark artifacts. `--skip-release-build` is useful for a quick
feature check, but it is not enough for publishable ToplingDB numbers. For
grant or README packages, write the full preflight marker consumed by the
package gate:
