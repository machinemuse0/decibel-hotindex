...
- ToplingDB native builds are long-running and belong in a separate cached
  preflight, not the fast RocksDB CI lane. On macOS/arm64, partial feature
  checks pass and the local compatibility patch clears multiple Darwin compile
  failures, but full release preflight still has not produced `librocksdb.dylib`;
  do not publish ToplingDB performance claims until
  `scripts/toplingdb-preflight.sh` succeeds without `--skip-release-build` and
  writes `toplingdb-preflight.ok`.
