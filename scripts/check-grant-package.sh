...
Options:
  ...
  --admin-bin <path>           Override decibel-admin path used for checksum comparison.
...
resolve_admin_bin() {
  ...
  if [[ -n "${DECIBEL_ADMIN_BIN:-}" ]]; then
    [[ -x "$DECIBEL_ADMIN_BIN" ]] || die "missing executable from DECIBEL_ADMIN_BIN: $DECIBEL_ADMIN_BIN"
    echo "$DECIBEL_ADMIN_BIN"
    return
  fi

  if command -v decibel-admin >/dev/null 2>&1; then
    command -v decibel-admin
    return
  fi

  local candidate
  for candidate in \
    "$repo_root/target/rocksdb/release/decibel-admin" \
    "$repo_root/target/release/decibel-admin" \
    "$repo_root/target/debug/decibel-admin"; do
    if [[ -x "$candidate" ]]; then
      echo "$candidate"
      return
    fi
  ...
  die "decibel-admin is required for --require-toplingdb checksum comparison; pass --admin-bin"
}
...
if grep -R '"status"[[:space:]]*:[[:space:]]*"fail"' "$dataset_root"/reports/bench-*.json >/dev/null; then
  ...
  if [[ "$allow_engineering_smoke" -eq 0 ]]; then
    ...
    if grep -RL '"methodology_status"[[:space:]]*:[[:space:]]*"publishable_candidate"' "$dataset_root"/reports/bench-*.json >/dev/null; then
      die "all benchmark reports must be marked methodology_status=publishable_candidate"
    fi
  fi
...
if [[ "$require_toplingdb" -eq 1 ]]; then
  resolved_admin_bin="$(resolve_admin_bin)"
  "$resolved_admin_bin" compare-checksum \
